# Self-contained image: compile React in Node, serve static assets from FastAPI.
# Node is not a production process.
FROM public.ecr.aws/docker/library/node:22-bookworm-slim AS frontend
WORKDIR /src/frontend
COPY frontend/package.json frontend/package-lock.json ./
RUN npm ci
COPY frontend ./
RUN npm run build -- --outDir /web
...
COPY --from=frontend /web ./natalia/web
RUN test -f natalia/web/index.html && test -d natalia/web/assets && pip install --no-deps .
USER 10001:10001
EXPOSE 8000
